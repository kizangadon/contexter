...
/// Holds the names of all 9 column families.
///
/// Each field stores the static CF name string. Use
/// [`RocksDbBackend::cf`] to resolve a name to a [`ColumnFamily`] reference.
#[derive(Debug, Clone)]
...
/// Configuration for opening a [`RocksDbBackend`].
#[derive(Debug, Clone)]
pub struct RocksDbConfig {
    ...
    /// [`StorageBackend::checkpoint()`] call, dramatically improving write
...
impl RocksDbBackend {
    ...
    pub fn open_with_config(config: RocksDbConfig) -> EngineResult<Self> {
        ...
        let cf_configs: [(&str, DBCompressionType, u64, bool, Option<i32>); 9] = [
            (
                CF_MEMORY_ITEMS,
    ...
    /// # Panics
    ///
    /// Panics if the named column family does not exist. This only happens
    /// if the constructor failed to create the CF, which is guaranteed to
    /// succeed for the 9 canonical CFs.
...
impl StorageBackend for RocksDbBackend {
    ...
    fn create_session(&self, session: NewSession) -> EngineResult<Session> {
        ...
        self.db
            .put_cf(self.cf(self.cfs.sessions), key.as_bytes(), value)?;
    ...
    fn list_sessions(&self, filter: &SessionFilter) -> EngineResult<Vec<Session>> {
        ...
        let iter = self
            .db
            .iterator_cf(self.cf(self.cfs.sessions), IteratorMode::Start);
    ...
    fn update_session(&self, id: Uuid, patch: &SessionPatch) -> EngineResult<Session> {
        ...
        let mut session: Session = serde_json::from_slice(&existing)?;
    ...
    fn delete_session(&self, id: Uuid) -> EngineResult<()> {
        let key = Self::session_key(&id);
        self.db
            .delete_cf(self.cf(self.cfs.sessions), key.as_bytes())?;
        self.maybe_flush_wal()?;
        Ok(())
    }
    ...
    fn get_setting(&self, key: &str) -> EngineResult<Option<String>> {
        ...
        match self
            .db
            .get_cf(self.cf(self.cfs.sessions), db_key.as_bytes())?
        {
            Some(bytes) => {
...
mod tests {
    ...
    fn test_engine_init_creates_cfs() {
        ...
        // Verify all 9 CFs are accessible.
    ...
    fn test_audit_append_query() {
        ...
        backend
            .append_audit_entry(&NewAuditEntry {
                ...
                actor: Some("user-1".into()),
                changes: Some(serde_json::json!({"content": "updated"})),
            })
            .expect("append audit");
